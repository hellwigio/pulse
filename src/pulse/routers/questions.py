...
@questions_bp.get("")
def get_questions():
    records = db.session.scalars(select(Question).order_by(Question.id))
...
def delete_question(id: int):
    ...
    has_category = db.session.scalar(select(Category.id).where(Category.question_id == id).limit(1))
    if has_answers is not None or has_category is not None:
        abort(409, description="Delete related answers and category first")
